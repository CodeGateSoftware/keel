...
(`execution.sizing`), runs the twenty-one un-overridable §14 hard rails (`execution.guards.check`)
...
bid/ask fails closed with a distinct reason. It sits BESIDE the twenty-one rails, not among
...
def _entry_spread_gate(
    ...
    **Where it sits, and why.** AFTER `guards.check` and AFTER the preview: guards are
    ...
    routing-time check BESIDE the twenty-one rails, not a numbered rail, and it consumes the
