...
| worked example (b=2, κ=0.10, p=45% → ρ*≈12%, notional 240%) | §3.4 | ✓ exact |
| κ / `p_be` table | §3.3 | ✓ every cell |
| `n ≈ 1.55/δ²` | eq. 6 | ✓ `(z₀.₉₅+z₀.₈₀)²/4 = 1.546` |
| sample-size table and durations | §4.2 | ✓ every cell |
...
| the note | #368 |
|---|---|
| §3.1 stake-vs-risk; tighter stop ⇒ bigger position | "One translation warning before comparing" |
...
treating 100 pooled trades as 100 observations. The note's §5.2 warns that naive pooling assumes
