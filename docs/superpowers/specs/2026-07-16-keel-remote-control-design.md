# keel — Remote Control & Notifications — Design Spec
...
**Relates to:** `2026-07-15-keel-autotrade-design.md` (main spec; §4 confirm/bypass, §14 security).
...
Let the user, from the **Claude mobile app**, (a) **receive notifications** from the local keel agent
...
```
 keel daemon (Python, deterministic)          Operator session (Claude Code, `--remote-control`)
   ...
   • polls for approve/reject decision  ◀───────────    runs `keel approve|reject|kill|resume <id>`
```
...
- **Operator session runbook** (not code — a documented Claude Code session + a short standing prompt):
  "connect Remote Control; watch `keel pending`; when a new item appears, `PushNotification` a one-line
  summary; on the user's phone reply, run exactly `keel approve <id>` or `keel reject <id>` (or
