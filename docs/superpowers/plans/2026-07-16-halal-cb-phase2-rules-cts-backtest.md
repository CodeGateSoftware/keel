# halal-cb Phase 2 — Rules, CTS Engine, Backtest & Paper Gate — Implementation Plan
...
- **Shared strategy types live in `halal_cb/strategy/rules/base.py`** (Task 1) — all rules/engine/backtest import from there.
- Import Phase-1 types from `halal_cb.types`; analysis from `halal_cb.analysis.*`; persistence from `halal_cb.data.repository`.
...
**Files:** Create `halal_cb/strategy/__init__.py`, `halal_cb/strategy/rules/__init__.py`, `halal_cb/strategy/rules/base.py`, `tests/strategy/test_base.py`.
...
**Acceptance:** all downstream tasks can `from halal_cb.strategy.rules.base import Rule, Setup, Signal, Trade, Action`; `Setup.rr` correct; ABC enforced.

---

### Task 2 — CTS confluence scorer  `[wave B]`

**Files:** Create `halal_cb/strategy/indicators_cts.py`, `tests/strategy/test_cts.py`.
...
**Files:** Create `halal_cb/strategy/rules/pullback_continuation.py`, `tests/strategy/test_pullback.py`.
...
**Files:** Create `halal_cb/strategy/rules/rsi_meanrev.py`, `tests/strategy/test_rsi_meanrev.py`.
...
**Files:** Create `halal_cb/strategy/rules/dca.py`, `tests/strategy/test_dca.py`.
...
**Files:** Create `halal_cb/strategy/backtest.py`, `tests/strategy/test_backtest.py`.
...
**Files:** Create `halal_cb/strategy/engine.py`, `tests/strategy/test_engine.py`.
...
**Files:** Create `halal_cb/strategy/paper.py`, `tests/strategy/test_paper.py`.
...
**Files:** Create `halal_cb/strategy/promotion.py`, `tests/strategy/test_promotion.py`.
