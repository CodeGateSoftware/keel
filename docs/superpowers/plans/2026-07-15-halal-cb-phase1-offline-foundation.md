# halal-cb Phase 1 — Offline Foundation — Implementation Plan
...
**Goal:** Build the fully-offline foundation of the halal-cb agent — SQLite persistence, historical CSV
...
**Architecture:** Python package `halal_cb/` with focused single-responsibility modules. Pure-logic
...
- **stdlib `sqlite3` only** — no ORM. One DB file `halal_cb.db` (git-ignored).
...
- **Shared value types** live in `halal_cb/types.py` (created in Task 1) — all tasks import from there.
...
- Create: `halal_cb/__init__.py`, `halal_cb/types.py`, `halal_cb/config.py`
- Create: `tests/__init__.py`, `tests/test_config.py`, `tests/conftest.py`
- Create: `halal_cb/{data,analysis,strategy,execution}/__init__.py` (empty package dirs)
...
**Files:** Create `halal_cb/data/db.py`, `halal_cb/data/repository.py`, `tests/data/test_db.py`, `tests/data/test_repository.py`.

**Interfaces — Consumes:** `halal_cb.types` (Candle, Granularity, Side).
**Interfaces — Produces:**
- `db.py`: `def connect(path: str | Path = "halal_cb.db") -> sqlite3.Connection` (row_factory=Row, `PRAGMA foreign_keys=ON`); `def migrate(conn) -> None` (idempotent — creates all §6 tables + indexes if absent, tracks `schema_version`).
...
**Files:** Create `halal_cb/analysis/candles.py`, `tests/analysis/test_candles.py`.
**Consumes:** `halal_cb.types.Candle`. **Produces (pure functions on `Candle`/`list[Candle]`, all `k`/`m` as params):**
...
**Files:** Create `halal_cb/analysis/levels.py`, `tests/analysis/test_levels.py`.
**Consumes:** `halal_cb.types.Candle`. **Produces:**
...
**Files:** Create `halal_cb/analysis/regime.py`, `tests/analysis/test_regime.py`.
**Consumes:** `halal_cb.types.Candle`, `levels.swing_highs/lows` (import from Task 4 — declare the dependency; if worktrees race, stub the two functions' signatures).
**Produces:**
...
**Files:** Create `halal_cb/analysis/indicators.py`, `tests/analysis/test_indicators.py`.
**Consumes:** `halal_cb.types.Candle`. Uses `float` internally; accepts `list[Candle]` or `list[float]` of closes.
**Produces:**
...
**Files:** Create `halal_cb/data/cb_client.py`, `tests/data/test_cb_client.py`, `tests/fixtures/cb_*.json`.
**Consumes:** `halal_cb.types` (Candle, Granularity), `config.load_secrets`. **Produces** a thin, injectable wrapper (accepts a transport/`RESTClient` so tests inject a fake — NO live calls in tests):
...
**Files:** Create `halal_cb/data/csv_import.py`, `tests/data/test_csv_import.py`, `tests/fixtures/*.csv` (trimmed real samples).
...
**Files:** Create `halal_cb/analysis/pnl.py`, `tests/analysis/test_pnl.py`.
...
**Files:** Create `halal_cb/data/market_feed.py`, `tests/data/test_market_feed.py`.
