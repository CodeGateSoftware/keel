...
def test_every_runs_as_resolves():
    ...
    not guessed) or a `docs/experiments/*.py` driver that exists on disk -- never a string
    that used to be true.

    Mutation-verified: pointing one entry's `runs_as` at `"keel research not-a-real-command"`
    made this fail with `AssertionError: keel research not-a-real-command does not resolve
    to a registered CLI command`; see the commit message for the exact diff and output.
    """
