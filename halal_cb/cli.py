"""halal-cb command-line interface (P3 Task 9).
...
from halal_cb.analysis import pnl as pnl_analysis
from halal_cb.config import Config, load_config
from halal_cb.data import market_feed
from halal_cb.data.csv_import import import_dir
...
from halal_cb.strategy import backtest as backtest_mod
from halal_cb.strategy import promotion as promotion_mod
from halal_cb.types import Granularity

DISCLAIMER = (
    "halal-cb is a personal tool, not financial advice and not religious (Shariah) advice. "
    "Consult a qualified financial advisor and a knowledgeable scholar before trading. "
    "You are solely responsible for your own trading decisions."
)

DEFAULT_DB_PATH = "halal_cb.db"
...
@click.option(
    ...
    show_default=True,
    help="Passphrase-gate state file (see `halal_cb.security.authz`).",
)
@click.pass_context
def cli(ctx: click.Context, db_path: str, config_path: str, authz_path: str) -> None:
    """halal-cb: an offline-first, halal, guard-railed Coinbase auto-trading agent."""
